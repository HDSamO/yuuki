...
class TopicController {
  ...
  Future<TopicResult> addTopic(MyUser user, Topic topic) async {
    try {
      final DocumentReference userRef = _firestore.collection("users").doc(user.id);
      ...
      if (userSnapshot.exists) {
        ...
        final DocumentReference topicRef = await _firestore.collection("topics").add(topic.toFirestore());
        final String topicId = topicRef.id;
        topic.setId(topicId);

        await topicRef.update({'id': topicId}); // Update topic with generated ID
  ...
  Future<UserTopicResult> addTopicToAuthor(String topicId) async {
    try {
      final DocumentReference topicRef = _firestore.collection("topics").doc(topicId);
      final DocumentSnapshot topicSnapshot = await topicRef.get();

      if (topicSnapshot.exists) {
        final Topic topic = topicSnapshot.data()! as Topic; // Safe cast after existence check

        final UserTopic userTopic = UserTopic.fromTopic(topic);
        userTopic.setView(1);
        userTopic.setLastOpen(DateTime.now().millisecondsSinceEpoch);

        final String userId = topic.getAuthor!; // Safe access after existence check

        final DocumentReference userRef = _firestore.collection("users").doc(userId);
        final CollectionReference createdTopicsCollection = userRef.collection("userTopics");
  ...
  Future<List<Topic>> getTopTopicsByViews() async {
    try {
      final CollectionReference topicsCollection = _firestore.collection("topics");
      final Query topTopicsQuery = topicsCollection.where("private", isEqualTo: "false") 
                                                  .orderBy("views", descending: true) 
                                                  .limit(10);

      final QuerySnapshot querySnapshot = await topTopicsQuery.get();

      final List<Topic> topTopics = querySnapshot.docs.map((doc) => doc.data()! as Topic).toList();
      return topTopics;
    } on FirebaseException catch (e) {
      throw Exception("Error fetching top topics: ${e.message}"); // Re-throw as a generic Exception
    }
  }
  ...
  Future<List<UserTopic>> getRecentTopics(MyUser user) async {
    try {
      ...
      if (userSnapshot.exists) {
        final CollectionReference userTopicsCollection = userRef.collection("userTopics");
        final Query recentTopicsQuery = userTopicsCollection
            .orderBy("lastOpen", descending: true); // Order by lastOpen in descending order
      ...
      throw Exception("Error fetching recent topics: ${e.message}"); // Re-throw as a generic Exception
    }
  }
  ...
  Future<Topic?> getTopicById(String topicId) async {
    try {
      ...
      throw Exception("Error fetching topic: ${e.message}"); // Re-throw as a generic Exception
    }
  }
  ...
  try {
    ...
    final DocumentReference userRef = FirebaseFirestore.instance.collection("users").doc(user.id);
    final DocumentSnapshot userSnapshot = await userRef.get();

    if (userSnapshot.exists) {
      if (userSnapshot.id != topic.getAuthor) {
        return TopicResult(success: false, errorMessage: "User is not authorized to update the topic");
      }
    ...
    return TopicResult(success: false, errorMessage: "Error updating topic: ${e.message}"); // Re-throw as a generic Exception
  }
}
  ...
  Future<TopicResult> updateTopicOfUser(String topicId) async {
    try {
      ...
      if (topicSnapshot.exists) {
        ...
        final UserTopic userTopic = UserTopic.fromTopic(topic);
        userTopic.setLastOpen(DateTime.now().millisecondsSinceEpoch); // Update lastOpen

        final DocumentReference userTopicRef = createdTopicsCollection.doc(topicId);
    ...
    try {
      ...
      if (userSnapshot.exists) {
        ...
        if (topicSnapshot.exists) {
          ...
          if (authorId == user.id) { // Check against user ID
            await topicRef.delete();
            return TopicResult(success: true); // Topic deleted successfully
          } else {
            return TopicResult(success: false, errorMessage: "User does not have permission to delete the topic");
          }
        } else {
          return TopicResult(success: false, errorMessage: "Topic not found");
        }
      ...
      return TopicResult(success: false, errorMessage: "Error deleting topic: ${e.message}"); // Handle Firebase errors
    }
  }

  Future<UserTopicResult> deleteUserTopicById(MyUser user, String userTopicId) async {
    try {
      final DocumentReference userRef = FirebaseFirestore.instance.collection("users").doc(user.id); // Use user.id directly
      final DocumentSnapshot userSnapshot = await userRef.get();

      if (userSnapshot.exists) {
        final DocumentReference userTopicRef = userRef.collection("userTopics").doc(userTopicId);
        await userTopicRef.delete();
        return UserTopicResult(success: true); // User topic deleted successfully
      } else {
        return UserTopicResult(success: false, errorMessage: "User not found");
      }
    } on FirebaseException catch (e) {
      return UserTopicResult(success: false, errorMessage: "Error deleting user topic: ${e.message}"); // Handle Firebase errors
    }
  }
  ...
  try {
    final DocumentReference userRef = FirebaseFirestore.instance.collection("users").doc(user.id); // Use user.id directly
    final DocumentSnapshot userSnapshot = await userRef.get();
    CollectionReference userTopicsCollection = userRef.collection("userTopics");
    if (userSnapshot.exists) {
      final DocumentReference userTopicRef = userRef.collection("userTopics").doc(topicId);
      final DocumentSnapshot userTopicSnapshot = await userTopicRef.get();
      ...
      if (userTopicSnapshot.exists) {
        ...
        if (topicSnapshot.exists) {
          ...
          if (updateRequired) {
            ...
            userTopic = UserTopic.fromTopic(updatedTopic); // Implement updateFromTopic method
            await userTopicRef.update(userTopic.toFirestore() // Update userTopic document
              // ..remove('view') 
              // ..remove('lastOpen')
              );
          }
